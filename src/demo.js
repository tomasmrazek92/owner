...
$(document).ready(() => {
  ...
  function internalValidation(el) {
    ...
    el.find(':input:visible')
      .add('[data-form="check-checkbox"]:visible :input[type="checkbox"]')
      .not('select')
    ...
    if (showSchedule) {
      ...
      if (window.defaultSchedulerUrl) {
        ...
        function replaceMeetingEmbed() {
          ...
          const $target = $('.demo-form-box').first();
          const $fallback = $(success).closest('section');
          const $scrollTo = $target.length ? $target : $fallback;

          $('html, body').animate(
            {
              scrollTop: $scrollTo.offset().top - 160,
            },
            600
          );
        }
        ...
        if (success.attr('data-form-open') === 'on-click') {
          $('[data-form-btn="open-scheduler"]').on('click', function () {
            replaceMeetingEmbed();
          });
        } else {
          replaceMeetingEmbed();
  ...
  function initMultistep() {
    const mutlistepTypes = {
      ...
      'quiz-grader': {
        ...
        1: ['goals'],
        2: ['most-important'],
        3: ['first-name', 'last-name', 'role', 'locations'],
        4: ['email', 'cellphone'],
      },
    };
    ...
    function updateSteps(direction = 'next') {
      ...
      function updateProgressBar(currentStep) {
        let customCounter = $('.quiz-progressbar-wrapper-2');
        if (customCounter.length) {
          customCounter.css('display', currentStep >= 1 ? 'flex' : 'none');

          const total = $('.quiz-progressbar-wrapper-2 .quiz-single-progress').length;
          const percent = ((currentStep + 1) / total) * 100;

          $('.stats-svg-2 .progress').css(
            'stroke-dashoffset',
            `calc(188.5 - (188.5 * ${percent}) / 100)`
          );
          $('.circular-progress-point').text(`${currentStep + 1} of ${total}`);

          if ($(window).width() < 992) {
            $('.quiz-progressbar-wrapper-2 .quiz-single-progress').each(function (index) {
              $(this).css('display', index === currentStep ? 'flex' : 'none');
            });
          } else {
            $('.quiz-progressbar-wrapper-2 .quiz-single-progress').css('display', '');
            $('.quiz-pro-bar-2').each(function (index) {
              $(this).toggleClass('active', index < currentStep);
            });
            $('.quiz-single-title').each(function (index) {
              $(this).toggleClass('active', index < currentStep);
            });
          }
        }
      ...
      $(window).on('resize', function () {
        updateProgressBar(window._currentStep || 0);
      });
...
// API base config
const BASE_URL = 'https://olympus-edge-proxy.owner-preview.dev/api';
const headers = {
...
// Generic fetch helper (adds credentials + better error logging)
async function fetchJSON(url, options = {}) {
  const response = await fetch(url, {
    // IMPORTANT: send cookies/session if the API relies on them
    credentials: 'include',
    ...options,
  ...
  // If not OK, read body (often contains useful error info)
  if (!response.ok) {
    ...
    throw new Error(`${options.method || 'GET'} failed: ${response.status}`);
  }

  // Some APIs return empty body on 204 etc.
...
// API calls
const createPipeline = (googlePlaceId, source = '') =>
  fetchJSON(`${BASE_URL}/growth/v1/pipelines/brand-from-place-id`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ googlePlaceId, source }),
  });

const getPipeline = (pipelineId) =>
  fetchJSON(`${BASE_URL}/growth/v1/pipelines/${pipelineId}`, { headers });

// ✅ FIX: include headers + credentials via fetchJSON
const getBrandReport = (brandId) =>
  fetchJSON(`${BASE_URL}/growth/v2/brands/${brandId}/report`, { headers });

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// Retry with cap (safe version)
async function refetchReport(brandId, maxAttempts = 8, delay = 3000) {
  let report;

  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    console.log(`Fetching report (attempt ${attempt})...`);
    report = await getBrandReport(brandId);
    ...
    console.log(`Report status: ${status}. Retrying in ${delay}ms...`);
    if (attempt < maxAttempts) await sleep(delay);
  }

  console.warn('Max attempts reached. Returning last report.');
  return report;
}

// Infinite-ish loop but with safety cap
async function refetchReportUntilComplete(brandId, delay = 3000, maxMinutes = 3) {
  const maxAttempts = Math.ceil((maxMinutes * 60 * 1000) / delay);

  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    console.log(`Fetching report... (attempt ${attempt}/${maxAttempts})`);

    const report = await getBrandReport(brandId);
    const status = report?.metadata?.status;
    ...
    // Optional: stop if API returns failed
    if (status === 'failed') {
  ...
  throw new Error('Report did not complete within the allowed time.');
}

function findValueByKey(obj, searchKey) {
  ...
  for (let key in obj) {
    ...
    if (val && typeof val === 'object') {
      const result = findValueByKey(val, searchKey);
...
function countIncompleteTasks(data) {
  let count = 0;

  if (Array.isArray(data)) {
    for (let item of data) count += countIncompleteTasks(item);
  } else if (typeof data === 'object' && data !== null) {
    if ('complete' in data && data.complete === false) count++;
    for (let key in data) count += countIncompleteTasks(data[key]);
  }
...
jQuery(document).ready(function ($) {
  ...
  let dotsTimer = setInterval(() => {
    dots = (dots + 1) % 4;
    const text = `Scanning your site to increase your online sales ${'.'.repeat(dots)}`;

    document.querySelectorAll('.quiz-content-inner-report .h24').forEach((el) => {
      el.textContent = text;
    });
  }, 300);

  function stopDots(optionalText) {
    if (dotsTimer) {
  ...
  window.addEventListener('stepChanged', function (e) {
    if (e.detail.step === 1) {
      ...
      const resname = $('#restaurant-name-info').val();

      if (window._lastProcessedResname === resname) return;
      window._lastProcessedResname = resname;

      (async () => {
        try {
          document.querySelector('.h18.truncate').textContent = resname;
          // 1) Create pipeline
          const created = await createPipeline(placeid);
          const pipelineExecutionId = created?.pipelineExecutionId;

          if (!pipelineExecutionId) {
            console.error('No pipelineExecutionId found', created);
            return;
          }

          console.log('Pipeline Execution ID:', pipelineExecutionId);

          // 2) Poll pipeline until brandId appears
          let brandId = null;
          let attempts = 0;
          const maxAttempts = 15; // 15 * 5s = 75s

          while (!brandId && attempts < maxAttempts) {
            const pipeline = await getPipeline(pipelineExecutionId);
            const artifacts = pipeline?.artifacts;
            const status = pipeline?.status;

            brandId = artifacts?.brandId || null;
            console.log(`Attempt ${attempts + 1}: status=${status}, brandId=${brandId}`);

            if (brandId) break;
            if (status === 'failed') throw new Error('Pipeline failed to complete.');

            await sleep(5000);
            attempts++;
          }

          if (!brandId) throw new Error('BrandId not generated after polling attempts');

          console.log('Brand ID found:', brandId);

          await new Promise((r) => setTimeout(r, 10000));

          // 3) Poll report until complete
          const report = await refetchReportUntilComplete(brandId, 2000, 3);
          ...
          const website = findValueByKey(report, 'website') || '';
          const problems = countIncompleteTasks(report);

          // Defensive checks
          const overallScore = report?.scores?.overall?.score ?? 0;
          const overallMax = report?.scores?.overall?.max ?? 100;
          const eGPV = metadata?.egpv ?? 0;
          const isDisqualified = metadata?.isDisqualified ?? false;

          console.log('isDisqualified: ' + isDisqualified);

          const boostsales = (1 - overallScore / 100) * eGPV * 12;
          const boostsales_rounded = Math.round(boostsales / 10) * 10;
          const formatted = boostsales_rounded.toLocaleString('en-US');
          ...
          // Update DOM
          document.querySelector('.report-container > .h18').textContent = `${name} Grader Report`;
          document.querySelector('.highlighted-sub').textContent = `${name} ${website}`.trim();
          document.querySelector(
            '.highlighted-info .h16'
          ).textContent = `We found ${problems} problems with your online presence`;

          $('.report-container .h20').html(
            `Fixing them with Owner could boost your sales by <strong>$${formatted}/year</strong>`
          );

          $('.circular-progress-value').html(
            `${overallScore}<em class="small-text">/${overallMax}</em>`
          );

          $('.stats-svg .progress').css(
            'stroke-dashoffset',
            `calc(339.292 - (339.292 * ${overallScore}) / 100)`
          );
          ...
          $('.stats-svg .progress').css('stroke', strokeColor);
          $('.stats-result-status').html(statsStatus);

          // --- Search Results ---
          const searchScore = report?.scores?.searchResultsV2?.score ?? 0;
          const searchMax = report?.scores?.searchResultsV2?.max ?? 1;
          const searchPercent = (searchScore / searchMax) * 100;

          $('.search-result .score-main').text(`${searchScore}/${searchMax}`);
          $('.search-result .score-svg .progress').css(
            'stroke-dashoffset',
            `calc(151 - (151 * ${searchScore} / ${searchMax}))`
          );

          if (searchPercent <= 50) {
            $('.search-result .stats-item-info .p12').text('Poor').css('color', '#ff3b3b');
            $('.search-result .score-svg .progress').css('stroke', '#ff3b3b');
          } else if (searchPercent <= 80) {
            $('.search-result .stats-item-info .p12').text('Fair').css('color', 'orange');
            $('.search-result .score-svg .progress').css('stroke', 'orange');
          } else {
            $('.search-result .stats-item-info .p12').text('Good').css('color', 'green');
            $('.search-result .score-svg .progress').css('stroke', 'green');
          }

          // --- Website Experience ---
          const webScore = report?.scores?.guestExperience?.score ?? 0;
          const webMax = report?.scores?.guestExperience?.max ?? 1;
          const webPercent = (webScore / webMax) * 100;

          $('.website-experience .score-main').text(`${webScore}/${webMax}`);
          $('.website-experience .score-svg .progress').css(
            'stroke-dashoffset',
            `calc(151 - (151 * ${webScore} / ${webMax}))`
          );

          if (webPercent <= 50) {
            $('.website-experience .stats-item-info .p12').text('Poor').css('color', '#ff3b3b');
            $('.website-experience .score-svg .progress').css('stroke', '#ff3b3b');
          } else if (webPercent <= 80) {
            $('.website-experience .stats-item-info .p12').text('Fair').css('color', 'orange');
            $('.website-experience .score-svg .progress').css('stroke', 'orange');
          } else {
            $('.website-experience .stats-item-info .p12').text('Good').css('color', 'green');
            $('.website-experience .score-svg .progress').css('stroke', 'green');
          }

          // --- Local Listings ---
          const localScore = report?.scores?.reputation?.score ?? 0;
          const localMax = report?.scores?.reputation?.max ?? 1;
          const localPercent = (localScore / localMax) * 100;

          $('.locallistings .score-main').text(`${localScore}/${localMax}`);
          $('.locallistings .score-svg .progress').css(
            'stroke-dashoffset',
            `calc(151 - (151 * ${localScore} / ${localMax}))`
          );

          if (localPercent <= 50) {
            $('.locallistings .stats-item-info .p12').text('Poor').css('color', '#ff3b3b');
            $('.locallistings .score-svg .progress').css('stroke', '#ff3b3b');
          } else if (localPercent <= 80) {
            $('.locallistings .stats-item-info .p12').text('Fair').css('color', 'orange');
            $('.locallistings .score-svg .progress').css('stroke', 'orange');
          } else {
            $('.locallistings .stats-item-info .p12').text('Good').css('color', 'green');
            $('.locallistings .score-svg .progress').css('stroke', 'green');
          }

          // Profile image
          if (images?.profile?.url && images.profile.url !== 'null') {
            document.querySelector('.highlighted-img')?.setAttribute('src', images.profile.url);
            document.querySelector('.highlighted-img')?.setAttribute('srcset', images.profile.url);
          }

          // Competitors
          const competitors = report?.searchResultsV2?.metadata?.competitors || null;

          if (Array.isArray(competitors) && competitors.length) {
            competitors.sort((a, b) => (b.score ?? 0) - (a.score ?? 0));

            let html = "<div class='ranking-card-heading'>Who's beating you on Google</div>";
            ...
            competitors.forEach((comp, index) => {
              ...
              let colorClass = '';
              if (rank <= 3) colorClass = 'success';
              else if (rank <= 6) colorClass = 'warning';
            ...
            <div class="ranking-items">
              <div class="ranking-left">
                ...
                <div class="ranking-info">
                  <div class="ranking-res-name">${comp?.name ?? ''}</div>
                  <div class="ranking-stars">
                    <strong class="star-rating">${comp?.rating ?? ''}</strong> <strong>★</strong>
</div>
</div>
</div>
              <div class="ranking-no ${colorClass}">${rankText}</div>
</div>
          `;
            });

            document.querySelector('.ranking-card').innerHTML = html;
          } else {
            const el = document.querySelector('.ranking-card');
            if (el) {
              el.innerHTML = '';
          ...
          document.querySelector('.thankyou-animation-2').style.display = 'none';
          document.querySelector('.thankyou-report-inner-2').style.display = 'flex';

          // Exposed
          let fullReportLink =
            'https://grader.owner.com/report/' +
            brandId +
            '?name=' +
            name +
            '&city=Brooklyn&admin=true';

          if (isDisqualified) {
            $('.quiz-content-inner-report .h24').text("Thanks, we'll be in touch soon!");
            $('.quiz-report-content-desktop > div > .p16, .quiz-report-content > div > .p16').text(
              'In the meantime, review your full Report, get competitor insights, and learn how to grow your sales.'
            );
            $('.demo-form_success .steep-inner-quiz-report .button-quiz .quiz-button-text').text(
              'Get My Full Report'
            );
            $('.demo-form_success .steep-inner-quiz-report .button-quiz .quiz-button-text').attr(
              'href',
              fullReportLink
            );
          } else {
            $('.quiz-content-inner-report .h24').text('Book a Demo to get your full report');
            $('.quiz-report-content-desktop > div > .p16, .quiz-report-content > div > .p16').text(
              'Book a 20-minute demo with an Owner Specialist to review your full Report, get competitor insights, and learn how to grow your sales.'
            );
            $('.demo-form_success .steep-inner-quiz-report .button-quiz .quiz-button-text').text(
              'Book a Demo'
            );
            $('[demo-form]').attr('data-custom-redirect', fullReportLink);
          }

          document
            .querySelectorAll(
              '.demo-form_success .steep-inner-quiz-report .button-quiz, .quiz-report-content-desktop > div > .p16, .quiz-report-content > div > .p16'
            )
            .forEach((el) => {
          ...
          if (window.innerWidth <= 767) {
            $('.dark-bg-quiz').css('padding-bottom', '30rem');
          }
        } catch (error) {
          ...
          // Optional: show your error UI here

          document.querySelector('.thankyou-animation-2').style.display = 'none';
          document.querySelector('.quiz-report-content-desktop .button-quiz').style.display =
            'none';
          document.querySelector('.thankyou-report-inner-2').style.display = 'none';
          document.querySelector('.thankyou-report-error').style.display = 'flex';
          ...
          document
            .querySelectorAll(
              '.quiz-report-content-desktop > div > .p16, .quiz-report-content > div > .p16'
            )
            .forEach((el) => {
              el.style.display = 'flex';
            });
